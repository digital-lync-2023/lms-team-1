		...
		try {
			...
			if (authenticate.isAuthenticated()) {
				...
				if (downloadImage != null) {
					...
					profilePhoto = "data:image/png;base64," + encodeToString;
				} else {
					profilePhoto = output.get().getUserName().substring(0, 2).toUpperCase();
				}
				...
				UserResponseDto ld2 = new UserResponseDto(output.get().getUserId(), output.get().getUserName(),
						userDto.getUserEmail(), genJwtToken, output.get().getRole(), profilePhoto, uc);
